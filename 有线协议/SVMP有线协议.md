...
所述SVMP有线协议是由`协议缓冲区`格式，规定和使用生成的（也简称为“protobuf的”）。它用于促进客户端，服务器和VM之间的通信。每条消息都是两个“超类型”之一，一个`Request`或一个 `Response`。这两种消息类型充当包装器，包含各种“子类型”和额外信息之一。从客户端发送的任何内容都是a `Request`，发送给客户端的任何内容都是`Response`。请注意，消息并不总是意味着预期的回复; 即客户端可以发送某些`Request`消息而不期望一个 `Response`，反之亦然。

每个`Request`并`Response`有一个`type`描述其属性亚型，并有一个可选属性与该亚型相关联。例如，当用JSON编写时，旋转信息请求如下所示：
`{"type":"ROTATIONINFO","rotationInfo":{"rotation":0}}`
但是，某些消息只有一个没有附加属性的类型。例如，当用JSON编写时，屏幕信息请求如下所示：
`{"type":"SCREENINFO"}`

可以通过查看[协议定义](https://github.com/SVMP/svmp-protocol-def/blob/master/svmp.proto)来查看这些消息类型。类型是不言自明的，并根据它们使用的机制分开。
下面是协议序列的示例，其中客户端通过其REST API与Overseer进行身份验证，打开与服务器的WebSocket连接，并建立与虚拟机的连接：

![](../res/svmp-protocol-sequence.png)

此外，包含协议消息信息的示例服务器日志如下：
...
有关协议缓冲区的信息，请参阅“ [开发人员指南”](https://developers.google.com/protocol-buffers/docs/overview)。